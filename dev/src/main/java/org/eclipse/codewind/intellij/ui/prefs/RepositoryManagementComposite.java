...
public class RepositoryManagementComposite extends JPanel {
	...
	protected void createControl() {
		...
		JTextField description = new JTextField(); //, SWT.READ_ONLY | SWT.WRAP | SWT.MULTI);
		description.setText("");
		...
		learnMoreLink.setText("<a>" + CodewindUIBundle.message("RepoMgmtLearnMoreLink") + "</a>");
		//learnMoreLink.setLayoutData(new GridData(GridData.END, GridData.END, false, false, 1, 1));
		
		learnMoreLink.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent event) {
				try {
					 ...
					 */
					URL url = new URL(UIConstants.TEMPLATES_INFO_URL);
					com.intellij.ide.browsers.BrowserLauncher.getInstance().browse(url.toURI());
				} catch (Exception e) {
					Logger.log("An error occurred trying to open an external browser at: " + UIConstants.TEMPLATES_INFO_URL, e); //$NON-NLS-1$
				}
			}
		});
		...
		JButton addButton = new JButton(); // SWT.PUSH);
		addButton.setText(CodewindUIBundle.message("RepoMgmtAddButton"));
		//addButton.setLayoutData(new GridData(GridData.FILL, GridData.BEGINNING, false, false));
		
		removeButton = new JButton(); //this, SWT.PUSH);
		removeButton.setText(CodewindUIBundle.message("RepoMgmtRemoveButton"));
		...
		descLabel = new JLabel(); //detailsComp, SWT.NONE);
		descLabel.setFont(boldFont);
		descLabel.setText(CodewindUIBundle.message("RepoMgmtDescriptionLabel"));
		descText = new JTextField(); //detailsComp, SWT.READ_ONLY | SWT.MULTI | SWT.WRAP);
		descText.setText("");
		...
		styleLabel = new JLabel(); //detailsComp, SWT.NONE);
		styleLabel.setFont(boldFont);
		styleLabel.setText(CodewindUIBundle.message("RepoMgmtStylesLabel"));
		styleText = new JTextField();// detailsComp, SWT.READ_ONLY | SWT.MULTI | SWT.WRAP);
		styleText.setText("");
		...
		linkLabel = new JLabel(); //detailsComp, SWT.NONE);
		linkLabel.setFont(boldFont);
		linkLabel.setText(CodewindUIBundle.message("RepoMgmtUrlLabel"));
		...
		description.setText(CodewindUIBundle.message("RepoMgmtDescription"));
	...
	public IStatus updateRepos(IProgressMonitor monitor) {
		SubMonitor mon = SubMonitor.convert(monitor, CodewindUIBundle.message("RepoUpdateTask"), 100);
		MultiStatus multiStatus = new MultiStatus(CodewindCorePlugin.PLUGIN_ID, IStatus.ERROR, CodewindUIBundle.message("RepoMgmtUpdateError"), null);
		...
		for (RepositoryInfo info : repoList) {
			...
			if (entry == null) {
				...
				try {
					...
					multiStatus.add(new Status(IStatus.ERROR, CodewindCorePlugin.PLUGIN_ID, NLS.bind(CodewindUIBundle.message("RepoMgmtRemoveFailed"), info.getURL()), e));
				}
				...
				try {
					...
					multiStatus.add(new Status(IStatus.ERROR, CodewindCorePlugin.PLUGIN_ID, NLS.bind(CodewindUIBundle.message("RepoMgmtUpdateFailed"), info.getURL()), e));
				}
			}
			if (mon.isCanceled()) {
		...
		for (RepoEntry entry : repoEntries) {
			...
			if (info == null) {
				...
				try {
					...
					multiStatus.add(new Status(IStatus.ERROR, CodewindCorePlugin.PLUGIN_ID, NLS.bind(CodewindUIBundle.message("RepoMgmtAddFailed"), entry.url), e));
				}
			}
			if (mon.isCanceled()) {
				return Status.CANCEL_STATUS;
		...
		public String getStyles() {
			if (info != null) {
				...
				if (styles == null || styles.isEmpty()) {
					return CodewindUIBundle.message("GenericNotAvailable");
				}
	...
	private static class AddDialog extends TitleAreaDialog {
		...
		protected void configureShell(Shell newShell) {
			...
			newShell.setText(CodewindUIBundle.message("AddRepoDialogShell"));
		}
		...
		protected Control createDialogArea(Composite parent) {
			...
			setTitle(CodewindUIBundle.message("AddRepoDialogTitle"));
			setMessage(CodewindUIBundle.message("AddRepoDialogMessage"));
			...
			label.setText(CodewindUIBundle.message("AddRepoDialogUrlLabel"));
			...
			label.setText(CodewindUIBundle.message("AddRepoDialogNameLabel"));
			label.setLayoutData(new GridData(GridData.BEGINNING, GridData.CENTER, false, false));
			...
			label = new Label(composite, SWT.NONE);
			label.setText(CodewindUIBundle.message("AddRepoDialogDescriptionLabel"));
			label.setLayoutData(new GridData(GridData.BEGINNING, GridData.CENTER, false, false));
		...
		private boolean validate() {
			if (url == null || url.isEmpty()) {
				setErrorMessage(CodewindUIBundle.message("AddRepoDialogNoUrl"));
				return false;
			}
			if (name == null || name.isEmpty()) {
				setErrorMessage(CodewindUIBundle.message("AddRepoDialogNoName"));
				return false;
			}
			if (description == null || description.isEmpty()) {
				setErrorMessage(CodewindUIBundle.message("AddRepoDialogNoDescription"));
				return false;
			}
