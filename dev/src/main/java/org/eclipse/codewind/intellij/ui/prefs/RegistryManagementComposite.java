...
public class RegistryManagementComposite extends Composite {
	...
	private JButton addButton, removeButton, pushRegButton;
	//private Color gray;
	...
	protected void createControl() {
		...
		JTextArea description = new JTextArea(); //this, SWT.READ_ONLY | SWT.WRAP | SWT.MULTI);
		...
		learnMoreLink.setText("<a>" + CodewindUIBundle.message("RegMgmtLearnMoreLink") + "</a>"); //$NON-NLS-1$ //$NON-NLS-2$
		...
		// Spacer
		new JLabel();//this, SWT.NONE).setLayoutData(new GridData(GridData.FILL, GridData.FILL, false, false, 2, 1));
		
		// Create a composite for the table so can use TableColumnLayout
		JPanel tableComp = new JPanel(); //this, SWT.NONE);
		/*
		...
		// Table
		// create the table after we have the column names
		// regTable = new JTable(tableComp, SWT.BORDER | SWT.V_SCROLL | SWT.H_SCROLL | SWT.MULTI | SWT.FULL_SELECTION);
		/*
		...
		addressColumn.setText(CodewindUIBundle.message("RegMgmtAddressColumn"));
		addressColumn.setResizable(true);
		
		TableColumn usernameColumn = new TableColumn(regTable, SWT.NONE);
		usernameColumn.setText(CodewindUIBundle.message("RegMgmtUsernameColumn"));
		...
		if (supportsPushReg) {
			...
			namespaceColumn.setText(CodewindUIBundle.message("RegMgmtNamespaceColumn"));
			namespaceColumn.setResizable(true);
			
			pushColumn = new TableColumn(regTable, SWT.NONE);
			pushColumn.setText(CodewindUIBundle.message("RegMgmtPushRegColumn"));
		...
		*/
		...
		if (supportsPushReg)
		{
			tableColumnNames = {CodewindUIBundle.message("RegMgmtAddressColumn"), CodewindUIBundle.message("RegMgmtUsernameColumn"), CodewindUIBundle.message("RegMgmtNamespaceColumn"), CodewindUIBundle.message("RegMgmtPushRegColumn")};
		}
		else
		{
			tableColumnNames = {CodewindUIBundle.message("RegMgmtAddressColumn"), CodewindUIBundle.message("RegMgmtUsernameColumn")};
		}

		regTable = new JTable(null, tableColumnNames);
		...
		// Table buttons
		addButton = new JButton(); //this, SWT.PUSH);
		addButton.setText(CodewindUIBundle.message("RegMgmtAddButton"));
		//addButton.setLayoutData(new GridData(GridData.FILL, GridData.BEGINNING, false, false));

		pushRegButton = new JButton(); //this, SWT.PUSH);
		pushRegButton.setText(CodewindUIBundle.message("RegMgmtSetPushButton"));
		//pushRegButton.setLayoutData(new GridData(GridData.FILL, GridData.BEGINNING, false, false));
		
		removeButton = new Button(); //this, SWT.PUSH);
		removeButton.setText(CodewindUIBundle.message("RegMgmtRemoveButton"));
		...
		pushRegButton.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent event) {
		...
		String descText = supportsPushReg ? CodewindUIBundle.message("RegMgmtDescription") : CodewindUIBundle.message("RegMgmtLocalDescription");
	...
	private void createItems() {
		...
		for (RegEntry regEntry : regEntries) {
			...
			if (supportsPushReg) {
				...
				if (regEntry.isPushReg) {
					item.setText(3, CodewindUIBundle.message("RegMgmtPushRegSet"));
				}
			}
		}
	}
	...
	public IStatus updateRegistries(IProgressMonitor monitor) {
		...
		MultiStatus multiStatus = new MultiStatus(CodewindCorePlugin.PLUGIN_ID, IStatus.ERROR, CodewindUIBundle.message("RegMgmtUpdateError"), null);
		...
		for (RegistryInfo info : regList) {
			...
			if (entry == null) {
				...
				try {
					...
					multiStatus.add(new Status(IStatus.ERROR, CodewindCorePlugin.PLUGIN_ID, NLS.bind(CodewindUIBundle.message("RegMgmtRemoveFailed"), info.getAddress()), e));
				}
			}
			if (mon.isCanceled()) {
		...
		for (RegEntry entry : regEntries) {
			...
			if (info == null) {
				...
				try {
					...
					multiStatus.add(new Status(IStatus.ERROR, CodewindCorePlugin.PLUGIN_ID, NLS.bind(CodewindUIBundle.message("RegMgmtAddFailed"), entry.address), e));
				}
			}
			if (mon.isCanceled()) {
				return Status.CANCEL_STATUS;
			...
			if (entry.isPushReg) {
				try {
					...
					multiStatus.add(new Status(IStatus.ERROR, CodewindCorePlugin.PLUGIN_ID, NLS.bind(CodewindUIBundle.message("RegMgmtSetPushRegFailed"), info.getAddress()), e));
				}
			}
			if (mon.isCanceled()) {
				return Status.CANCEL_STATUS;
	...
	private class AddDialog extends TitleAreaDialog {
		...
		protected void configureShell(Shell newShell) {
			...
			newShell.setText(CodewindUIBundle.message("RegMgmtAddDialogShell"));
		}
		...
		protected Control createDialogArea(Composite parent) {
			...
			setTitle(CodewindUIBundle.message("RegMgmtAddDialogTitle"));
			setMessage(CodewindUIBundle.message("RegMgmtAddDialogMessage"));
			...
			label.setText(CodewindUIBundle.message("RegMgmtAddDialogAddressLabel"));
			...
			label.setText(CodewindUIBundle.message("RegMgmtAddDialogUsernameLabel"));
			label.setLayoutData(new GridData(GridData.BEGINNING, GridData.CENTER, false, false));
			...
			label = new Label(composite, SWT.NONE);
			label.setText(CodewindUIBundle.message("RegMgmtAddDialogPasswordLabel"));
			label.setLayoutData(new GridData(GridData.BEGINNING, GridData.CENTER, false, false));
			...
			if (supportsPushReg) {
				...
				pushButton.setText(CodewindUIBundle.message("RegMgmtAddDialogPushRegLabel"));
				...
				namespaceLabel.setText(CodewindUIBundle.message("RegMgmtAddDialogNamespaceLabel"));
		...
		private boolean validate() {
			if (address == null || address.isEmpty()) {
				setErrorMessage(CodewindUIBundle.message("RegMgmtAddDialogNoAddress"));
				return false;
			} else if (getRegEntry(address) != null) {
				setErrorMessage(CodewindUIBundle.message("RegMgmtAddDialogAddressInUse"));
				return false;
			}
			if (username == null || username.isEmpty()) {
				setErrorMessage(CodewindUIBundle.message("RegMgmtAddDialogNoUsername"));
				return false;
			}
			if (password == null || password.isEmpty()) {
				setErrorMessage(CodewindUIBundle.message("RegMgmtAddDialogNoPassword");
				return false;
			}
	...
	private class NamespaceDialog extends TitleAreaDialog {
		...
		protected void configureShell(Shell newShell) {
			super.configureShell(newShell);
			newShell.setText(CodewindUIBundle.message("RegMgmtNamespaceDialogShell"));
		}
		...
		protected Control createDialogArea(Composite parent) {
			setTitleImage(CodewindUIPlugin.getImage(CodewindUIPlugin.CODEWIND_BANNER));
			setTitle(CodewindUIBundle.message("RegMgmtNamespaceDialogTitle"));
			setMessage(CodewindUIBundle.message("RegMgmtNamespaceDialogMessage"));
